...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const meta = document.metadata as { storedName?: unknown; cloudinaryUrl?: unknown } | null;

    // Cloudinary path: ownership already verified above — safe to redirect.
    if (typeof meta?.cloudinaryUrl === 'string') {
      return NextResponse.redirect(meta.cloudinaryUrl);
    }

    // Local disk fallback (development / self-hosted with a persistent volume).
