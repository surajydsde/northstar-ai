...
const loadContext = async (state: AgentStateType) => {
  ...
  const [memoriesResult, docsResult] = await Promise.allSettled([
  ...
  if (memoriesResult.status === 'rejected') {
    logger.warn('context.memory_search_failed', {
      userId: state.userId,
      error: memoriesResult.reason instanceof Error ? memoriesResult.reason.message : String(memoriesResult.reason),
      cause:
        memoriesResult.reason instanceof Error && memoriesResult.reason.cause instanceof Error
          ? memoriesResult.reason.cause.message
          : undefined,
    });
  }
  if (docsResult.status === 'rejected') {
    logger.warn('context.doc_search_failed', {
      userId: state.userId,
      error: docsResult.reason instanceof Error ? docsResult.reason.message : String(docsResult.reason),
      cause:
        docsResult.reason instanceof Error && docsResult.reason.cause instanceof Error
          ? docsResult.reason.cause.message
          : undefined,
    });
  }

  const memories = memoriesResult.status === 'fulfilled' ? memoriesResult.value : [];
  const retrievedDocs = docsResult.status === 'fulfilled' ? docsResult.value : [];
